...
import { addCommentToInternalFeedbackThread, getInternalFeedbackById, getInternalFeedbackComments, updateInternalFeedbackType } from "@/utils/supabase/internalFeedback";
...
function CommentList({ feedbackId, feedbackComments }: {
  feedbackId: string,
  ...
  return <>
    ...
    <div className="space-y-4">
      ...
      <div className="w-full bg-slate-100  space-y-6 border px-6 p-4 pb-8 b-slate-300 overflow-hidden rounded-xl ">
        <CommentList feedbackId={feedbackThread.id} feedbackComments={feedbackComments} />
        ...
        {feedbackThread.status === 'closed' ||
          ...
          <AddComment addComment={addComment} feedbackId={feedbackThread.id} />
        )}
      </div>
    </div>
  </>;
