...
const CommentsList = () => {
  ...
  return <div className="space-y-4">
    ...
    {comments.map(comment => <div key={comment.id} className="grid grid-cols-[auto,1fr] items-start gap-3" >
      <UserAvatar userId={comment.user_id} size={24} />
      <div className="space-y-2">
        <UserFullName userId={comment.user_id} />
        <T.Small>{comment.text}</T.Small>
        <T.Subtle className="text-xs text-slate-400">{moment(comment.created_at).format('LLLL')}</T.Subtle>
      </div>
    </div>)}
  </div>
}
...
export const ProjectComments = () => {
  return <div className="space-y-4 w-[380px]">
    <T.H4>Comments</T.H4>
    <div className="space-y-2">
      <CommentInput />
      <CommentsList />
