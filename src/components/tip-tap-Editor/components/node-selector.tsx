...
export const NodeSelector: FC<NodeSelectorProps> = ({
  ...
  return (
    <div className="relative h-full">
      <button
        type="button"
        className="flex h-full items-center gap-1 whitespace-nowrap p-2 text-sm font-medium text-foreground hover:bg-muted active:bg-muted "
        ...
        <section className="fixed top-full z-[99999] mt-1 flex w-48 flex-col overflow-hidden rounded border  bg-background p-1 shadow-xl animate-in fade-in slide-in-from-top-1">
          {items.map((item, index) => (
            <button
              ...
              className="flex items-center justify-between rounded-sm px-2 py-1 text-sm text-foreground hover:bg-muted"
            >
              ...
              {activeItem.name === item.name && <LucideIcon name="Check" className="h-4 w-4" />}
            </button>
          ))}
        </section>
      )}
