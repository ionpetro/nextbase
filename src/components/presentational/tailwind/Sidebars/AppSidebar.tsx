...
function Links({ isUserAppAdmin, isExpanded, toggleIsExpanded }: LinksProps) {
  ...
  return (
    <div className={sidebarContainerClassName}>
      {user ? (
        <>
          <SidebarLink href="/dashboard" icon={<FiHome />} label="Dashboard" isExpanded={isExpanded} />
          {isUserAppAdmin && (
            <SidebarLink
              href="/app_admin"
              icon={<FiServer />}
...
export function AppSidebar({ isUserAppAdmin, isExpanded, toggleIsExpanded }: { isUserAppAdmin: boolean, isExpanded: boolean, toggleIsExpanded: (isExpanded: boolean) => void; }) {
  return (
